    ...
    <div className="text-left">
      ...
      {showPrice !== false && (
        <p className="text-sm text-gray-500">
          {product.stock === 0 ? (
            <span className="font-medium text-gray-400">Sold out</span>
          ) : (
            <>
  ...
  return (
    <section style={{ backgroundColor: bg }} className="px-6 py-10">
      {data.show_heading !== false && (
        <h2 className="mb-6 text-xl font-semibold text-gray-900">{data.heading || 'Featured products'}</h2>
      )}
      ...
      {data.show_view_all !== false && (
        <p className="mt-6 text-sm font-medium text-gray-700 underline">View all products</p>
      )}
    </section>
  );
}
