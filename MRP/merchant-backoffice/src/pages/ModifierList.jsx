  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (alive) setError(e.message || 'Failed to load modifiers');
      } finally {
  ...
  return (
    <div style={{ background: '#F4F4F4', height: 'calc(100vh - 56px)', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: "'Lato', sans-serif" }}>
      ...
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
        <h1 style={{ margin: '0 0 20px', fontSize: '26px', fontWeight: 700, color: '#282828', flexShrink: 0 }}>Modifier</h1>
        ...
        <div style={{ background: '#FFFFFF', borderRadius: '12px', border: '1px solid #E9E9E9', display: 'flex', flexDirection: 'column', overflow: 'hidden', flex: 1, minHeight: 0 }}>
          ...
          {error && !isLoading ? (
            <div style={{ padding: '40px', textAlign: 'center' }}>
              <p style={{ fontSize: '15px', fontWeight: 600, color: '#EF4444', marginBottom: '8px' }}>Failed to load modifiers</p>
            ...
            <div style={{ overflowY: 'auto', flex: 1, minHeight: 0 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', fontSize: '14px' }}>
                <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                  <tr>
                    <th style={{ ...TH, width: '22%' }}>Modifier Name</th>
                    <th style={{ ...TH, width: '39%' }}>Modifier Options</th>
                    <th style={{ ...TH, width: '39%' }}>Connected Catalog</th>
                  </tr>
                </thead>
                <tbody>
                  {isLoading ? (
                    ...
                    <tr><td colSpan={3} style={{ padding: 0, borderBottom: 'none' }}>
                      <EmptyState illustration={<EmptyIllustration />}
                        title={keyword ? 'No Search Results Found' : 'No Modifier Yet'}
                        description={keyword ? 'Try searching with a different term, okay?' : 'We couldn’t find any modifier from your connected store.'} />
                    </td></tr>
                  ) : (
                    pageRows.map(m => (
                      <tr key={m.id}
