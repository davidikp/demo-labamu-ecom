    ...
    <div style={{ padding: '40px 48px', maxWidth: '100%', margin: '0' }}>
      ...
      <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '1px solid #F3F4F6' }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          <TextField label="Street Address" value={businessAddress.street} onChange={e => updateAddress('street', e.target.value)} placeholder={companyData?.address || 'Jl. Contoh No. 123'} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
          <div>
            <TextField label="City" value={businessAddress.city} onChange={e => updateAddress('city', e.target.value)} placeholder={companyData?.city || 'Jakarta'} />
          </div>
          <div>
            <TextField label="Province / State" value={businessAddress.province} onChange={e => updateAddress('province', e.target.value)} placeholder={companyData?.province || 'DKI Jakarta'} />
          </div>
          <div>
            <TextField label="Postal Code" value={businessAddress.postalCode} onChange={e => updateAddress('postalCode', e.target.value)} placeholder={companyData?.postalCode || '12345'} />
          </div>
          <div>
            <TextField label="Country" value={businessAddress.country} onChange={e => updateAddress('country', e.target.value)} placeholder={companyData?.country || 'Indonesia'} />
          </div>
        </div>
      </div>
    </div>
