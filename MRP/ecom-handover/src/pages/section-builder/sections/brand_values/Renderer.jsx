  ...
  return (
    <section style={{ backgroundColor: bg }} className="px-6 py-10">
      {data.show_heading !== false && (
        <h2 className="mb-6 text-xl font-semibold text-gray-900">{data.heading || t('sectionBuilder:sections.brandValues.defaultHeading')}</h2>
      )}
      {values.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-6">
          {values.map((v) => (
            <div key={v.id} className="max-w-[180px] text-center">
              ...
              <p className="text-sm font-medium text-gray-900">{v.label || t('sectionBuilder:sections.brandValues.defaultValueLabel')}</p>
