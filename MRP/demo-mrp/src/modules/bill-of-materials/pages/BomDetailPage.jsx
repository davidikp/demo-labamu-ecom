...
const MATERIALS_GRID_COLUMNS = "minmax(200px, 2fr) minmax(120px, 1fr) 160px 100px 120px 90px 80px 120px";
              ...
              bom.materials.map((line, idx) => {
                ...
                return (
                  <div key={idx} style={detailTableRowStyle(MATERIALS_GRID_COLUMNS, idx === bom.materials.length - 1)}>
                    ...
                    <span style={{ fontSize: "var(--text-title-3)" }}>{line.quantity}</span>
                    <span style={{ fontSize: "var(--text-title-3)" }}>{line.unit || "-"}</span>
              ...
              {bom.materials?.map((line, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", alignItems: "center" }}>
                    <div style={{ display: "flex", flexDirection: "column" }}>
                      <span style={{ color: "var(--neutral-on-surface-primary)" }}>{line.name}</span>
                      <span style={{ fontSize: "11px", color: "var(--neutral-on-surface-tertiary)" }}>
                        {line.sku} · {line.quantity} {line.unit} × {formatIDR(unitPrice)} · {materialCostMethod()}
                      </span>
                    </div>
