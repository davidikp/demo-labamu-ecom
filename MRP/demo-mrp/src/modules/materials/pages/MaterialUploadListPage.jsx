...
import { MaterialUploadDetailModal } from "../components/MaterialUploadDetailModal.jsx";
...
export const MaterialUploadListPage = ({ onNavigate }) => {
  const [batches, setBatches] = useState(getMaterialUploads());
  ...
  useEffect(() => subscribeMaterialUploads(setBatches), []);
  ...
  return (
    <div style={{ height: "calc(100vh - 64px)", padding: "24px", boxSizing: "border-box", display: "flex", flexDirection: "column", gap: "24px", overflow: "hidden", minHeight: 0 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <Button variant="filled" leftIcon={AddIcon} onClick={() => onNavigate("materials_bulk-upload-new")}>
          New Upload
        </Button>
      </div>
                ...
                <div
                  ...
                  onClick={() =>
                    row.status === "Review" || row.status === "Mapping" || row.status === "Normalizing Data"
                      ? onNavigate("materials_bulk-upload-new", { resumeDraftId: row.id })
      ...
      <MaterialUploadDetailModal isOpen={!!selectedBatch} onClose={() => setSelectedBatch(null)} batch={selectedBatch} />
    </div>
  );
};
